...
export const generateOSINTPDF = async (eyeconDetails, truecallerDetails, mobNo, upiDetails, userPhoneNumber, breachDetails) => {
    try {
        ...
        const whatsAppDetails = await fetchWhatsAppDetails(mobNo);

        // Extract details from the response
        const {
            result: { status, is_business, profile_pic_url, business_name }
        } = whatsAppDetails;

        // Generate content dynamically based on fetched data
        ...
        if (business_name) {
            whatsAppContent.push(`Business Name: ${business_name}`);
        }

        if (profile_pic_url) {
            whatsAppContent.push(`Profile Picture: Available`);
        } else {
            whatsAppContent.push(`Profile Picture: Not Available`);
        }

        // Calculate section height dynamically
        const whatsAppHeight = Math.max(50, whatsAppContent.length * 10 + 20);

        // Check if a new page is needed
        if (yOffset + whatsAppHeight > pageHeight - 10) {
            doc.addPage();
            setBackgroundColor(); // Your existing function
            yOffset = 10;
        }

        // Add the WhatsApp section
        addStyledContainerWithMessage(
            doc,
            10,
            yOffset,
            ...
            whatsAppHeight,
            5,
            [7, 94, 84], // WhatsApp-themed color
            'WhatsApp Account Details',
            whatsAppContent,
            path.join(process.cwd(), 'public', 'icons', 'whatsapp-icon.png')
        );

        yOffset += whatsAppHeight + 10; // Update yOffset for subsequent sections
